...
interface QuoteRequestsTableProps {
  ...
  setSelectedRequest: (request: IQuote | null) => void;
}
...
export default function QuoteRequestsTable({ 
  ...
  const sortedRequests = [...quoteRequests].sort((a, b) => 
      ...
      <table className="min-w-full divide-y divide-gray-200">
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {sortedRequests.map((request) => (
            <tr key={request._id?.toString()} className="hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => setSelectedRequest(request)}>
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                ...
                <div className="text-sm text-gray-500">{new Date(request.createdAt).toLocaleTimeString('fr-FR')}</div>
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
                <span className={`
